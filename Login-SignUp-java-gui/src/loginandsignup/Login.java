...
public class Login extends javax.swing.JFrame {
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            String sql = "SELECT * FROM Usuario WHERE Email = ? AND Password = ?";  // Considerar usar hashing para la contraseña
            try (PreparedStatement stmt = conn.prepareStatement(sql)) {
                stmt.setString(1, email);
                stmt.setString(2, password);  // Deberías hashear la contraseña antes de verificar
                ...
                try (ResultSet rs = stmt.executeQuery()) {
                    if (rs.next()) {
                        ...
                        listaUsuario.setVisible(true);  // Muestra la ventana ListaUsuario
                        listaUsuario.pack();
                        listaUsuario.setLocationRelativeTo(null);
                    } else {
                        JOptionPane.showMessageDialog(this, "Invalid email or password.", "Failed", JOptionPane.ERROR_MESSAGE);
                    }
                }
            }
